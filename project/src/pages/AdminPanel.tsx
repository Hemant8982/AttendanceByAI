...
import { Users, Calendar, Download, Search, Filter } from 'lucide-react';
...
export default function AdminPanel() {
  ...
  const handleOvertimeAction = async (id: string, status: 'approved' | 'rejected') => {
    try {
      await api.updateOvertimeStatus(id, status);
  ...
  const tabs: { key: Tab; label: string; icon: typeof Users }[] = [
    ...
    { key: 'overtime', label: 'Overtime', icon: Filter },
  ];
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-500 mt-1">Manage employees, attendance, and overtime</p>
        </div>
        {tab === 'attendance' && (
          <button
            onClick={exportCSV}
